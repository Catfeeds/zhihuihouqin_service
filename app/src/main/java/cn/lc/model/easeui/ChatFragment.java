...
import cn.lc.model.easeui.init.Constant;
import cn.lc.model.easeui.init.DemoHelper;
...
//        dao = new M_UserInfoDao(getActivity().getApplicationContext());
...
//        List<SearcePersonInfoBean> list = dao.findAll();
//        LogUtil.log(list.size() + "================保存数据库条数===================");
//        SearcePersonInfoBean loginBean = dao.findBeanByHXName(message.getUserName().replace("m", ""));
        LogUtil.log(message.getUserName() + "======================onSetMessageAttributes==================");
        if (isRobot) {
            //set message extension
            message.setAttribute("em_robot_message", isRobot);
        }
        // 通过扩展属性，将userPic和userName发送出去。
//        String userPic = loginBean.fullImgPath;
//        if (!TextUtils.isEmpty(userPic)) {
//            message.setAttribute("userPic", userPic);
//        }
//        String userName = loginBean.userName;
